...
Рассмотрим классы для создания сложных виджетов:

--------------------------------------------------------------------------------

1. Класс **CTkComboBox()** - используется для создания выпадающего списка. Основные параметры:
- **values** - список элементов (строк), которые будут в выпадающем списке;
- **font** - шрифт;
- **fg_color** - цвет кнопки-стрелки для открытия списка;
...
- **state** - состояние списка (используйте readonly - только для чтения)
и другие.
...
- метод **get()** - позволяет получить значение, выбранное в выпадающем списке, и либо сохранить его в какую-нибудь переменную, либо сравнить с каким-либо значением (пример: _s = cmb.get()_ - сохранение, _if cmb.get() == '...'_ - для целых чисел);
